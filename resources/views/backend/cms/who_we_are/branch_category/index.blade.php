...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                <a href="{{ route('admin.add_branch_category') }}" class="btn btn-success btn-sm" title="Add New">
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th>Title</th>
                                <th>Status</th>
                                <th colspan="2" class="text-center bg-warning">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($branch_category as $item)
                            <tr>
                                <td>{{$loop->iteration}}</td>
                                <td>{{ $item->en_title }}</td>
                                <td>{{$item->status==0? 'Unpublished':'Published'}}</td>
                                <td class="d-flex justify-content-center">
                                    <a href="{{ route('admin.edit_branch_category', $item->id) }}" title="Edit Branch Category"><button class="btn btn-primary"><i class="zmdi zmdi-edit"></i>
                                            </button></a>
                                    <form action="{{ route('admin.delete_branch_category') }}" method="post" id="delete">
                                        @csrf
                                        <input type="hidden" name="branch_category_id" value="{{ $item->id }}">
